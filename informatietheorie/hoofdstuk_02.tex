...
\emph{
\begin{itemize}
  \item[(a)] Bereken de entropie van de bron. \emph{$\sum^{2^{max}^{(x)}}_{i=1}-p_i\cdot ld(p_i)=\sum^{2^{max}^{(6)}}_{i=1}-6_i\cdot ld(6_i)=$}
  \item[(b)] Bereken het gemiddeld aantal bronsymbolen per run-length codewoord. \emph{$44/9=4.889$}
  \item[(c)] Bereken het gemiddeld aantal bit van de Huffmancode per run-length woord. \emph{$0,9*4,889+0,1*4,889=0,9$}
  \item[(d)] Bereken het gemiddelde aantal bronsymbolen \emph{b} per Huffmansymbool \emph{$4,889/9=0,54$}.
\end{itemize}}
...
De bacterie E.Colis zitten meer verschillende permissen waardoor de complexitijd toeneemt.
